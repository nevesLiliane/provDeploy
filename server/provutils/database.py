...
def monet_db_connection(instance, port=config.MONETPORT):
    import pymonetdb
    connection = pymonetdb.connect(username="monetdb", password="monetdb", hostname="localhost", database=instance,
                                   port=port)
    return connection
...
def list_configurations():
    ...
    cursor.execute("SELECT name, description FROM container_image;")
...
def add_configuration_path(name, path, description, run_line, arch, creation_date="", schemaversion="", library="",
                           origin="", engine_version="", engine=""):
    cursor = get_db_cursor(config.CONTAINER_PROVENANCE)
    ...
    query += "select '{}', '{}', '{}', 0, 'a', '{}', '{}', '{}', '{}', '{}', id , '{}' , '{}' from container_image where execution_command like '%{}%' limit 1;".format(
        name, path, description, run_line, arch, creation_date, schemaversion, library, engine_version, engine,
        origin.strip())
...
def insert_execution(strategy, workflow_id):
    ...
    query = "INSERT INTO execution (timestamp,composition, wf_id) VALUES ('{}','{}', '{}');".format(
        datetime.datetime.now(), config.composition[strategy], workflow_id)
    # print(query)
    cursor.execute(query)
    cursor.execute("COMMIT;")
...
def save_workflow_description(name, activities, strategy, image=''):
    ...
    for a in activities:
        if isinstance(a['code'], list):  # this is hybrid hell
            for code in a['code']:
                query = "INSERT INTO workflow_execution(wf_id,activity, script, exec_command,image_id,exec_id ) VALUES ('{}','{}','{}','{}','{}','{}');".format(
                    id_wf, code, code, a['exec_command'], get_image_id_by_name(a['image'] if image == '' else image),
                    id_exec)
                cursor.execute(query)
        else:
            query = "INSERT INTO workflow_execution(wf_id,activity, script, exec_command,image_id,exec_id ) VALUES ('{}','{}','{}','{}','{}','{}');".format(
                id_wf, a['code'], a['code'], a['exec_command'],
                get_image_id_by_name(a['image'] if image == '' else image), id_exec)
            cursor.execute(query)
        # print(query)
...
def get_workflow_activity(script, workflow):
    ...
    query = "SELECT activity, script, exec_command, image.name FROM workflow_execution JOIN container_image on image_id=image.id where script='{}' and wf_id=(select max(id) from workflow where workflow='{}');".format(
        script, workflow)
    # print(query)
    cursor.execute(query)
    result = cursor.fetchone()
...
def get_default_provCollector():
    ...
    cursor = get_db_cursor(config.CONTAINER_PROVENANCE)
    cursor.execute(
        "SELECT image_path, execution_command FROM container_image where default_prov=1 and image_type='provCollector';")
    result = cursor.fetchone()
    if result is None:
...
def add_provCollector_path(name, path):
    ...
    cursor.execute(
        "INSERT INTO image(name, image_path, description, image_type) VALUES ({}, {}, 'hybrid data collection application','h');".format(
            name, path))
    cursor.execute("COMMIT;")
...
def get_default_database(port=config.MONETPORT):
    cursor = get_db_cursor(config.CONTAINER_PROVENANCE)
    cursor.execute(
        "SELECT image_path, execution_command FROM container_image where type='database' and default_prov=1;")
    result = cursor.fetchone()
    return result[0], result[1]
...
def get_config_by_id(id):
    cursor = get_db_cursor()
    cursor.execute(
        "SELECT id, name, image_path, description, default_prov, type, url, execution_command, config_file FROM container_image where id={};".format(
            id))
    result = cursor.fetchone()
...
def get_id_images_last_execution():
    ...
    for n in cursor.fetchall():
        id, name, image_path, description, default_prov_data, image_type, shub_url, execution_command, recipe_path = get_config_by_id(
            n)
...
def get_recipe_images_last_execution():
    ...
    cursor = get_db_cursor(config.CONTAINER_PROVENANCE)
    cursor.execute(
        "SELECT config_file, image_path FROM container c join container_image ci on c.image_id=ci.id where execution_id=(SELECT max(id) FROM execution);")
    for n in cursor.fetchall():
        # recipe, image_path = n
        configs.append(n)
        return configs
...
'''  
  SELECT  execution_id, config_file, image_path FROM container c join container_image ci on c.image_id=ci.id where 
                    execution_id in (SELECT max(e.id) FROM execution e join workflow_execution we on e.id = we.exec_id 
                    join workflow w on we.wf_id=w.id where workflow='SciPhy and e.id=114');
