...
def start_application2(configuration, application, exec_command):
    ...
    if exec_command == '':
        ...
        exec_command = run_line.split(" ")[
                           0] + " " + exec_command  # concatenando os comandos pois o usuário não sabe onde ficam guardadas as imagens
    start(path, exec_command, application)
    ...
    with open(input_file) as json_file:
        ...
        for p in data['new_conf']:
            ...
            if p['vendor'] == "singularity":
                ...
                if not connection.isPortInUse():
                    start_database()
                database.add_configuration_path(name, recipes_path + name.strip(), description, run_line,
                                                arch.split(":")[1], build_date, version_schema.split(":")[1],
                                                library.split(":")[1], origin.split(":")[1], eversion.split(":")[1],
                                                p['vendor'])
                ...
                if not connection.isPortInUse():
                    start_database()
                database.add_configuration_path(name, recipes_path + name.strip(), description, run_line,
                                                arch.split(":")[1], creation_date, version_schema.split(":")[1],
                                                library.split(":")[1], origin.split(":")[1], eversion.split(":")[1],
                                                p['vendor'])
            ...
            for code in codes:
                ...
                if len(codes) > codes.index(code) + 1:
                    ...
                    if isinstance(atual, list):  # se é hibrido
                        strategy = 'H'
                        atual = atual[
                            -1]  # é apenas o ultimo script de um bloco e o primeiro do proximo bloco que serão olhados e alterados em busca do gancho de um pro outro
                    ...
                    if not (search_str(path + atual, prox)):
                        print(
                            "The workflow is not well descripted in the configuration file, please review the description file and the scripts provided.")
                        exit();
                    else:
...
def wrap(key):
    if not connection.isPortInUse():
        start_database()
    recipes = database.get_recipe_images_by_execution_workflow_name(key)
    print(recipes)
...
def create_ro_recipes(recipes):
    ...
    for file, image_path in recipes:
        section_index = 0
        mb = False
        with open(file) as f:
            for line in f:
...
def access_database(database):
    if database == "container":
        ...
        print(
            database + " is not a valid option, please define if you want to access the workflow provenance database or the container provenance database")
